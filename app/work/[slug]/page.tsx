  ...
  return (
    <div className="min-h-screen bg-background text-foreground pt-32 md:pt-40 pb-10 md:pb-20 px-4 md:px-[60px] overflow-x-hidden">
      <div className="max-w-[1440px] mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 md:gap-10">
            ...
            <div className="flex gap-8 md:gap-10 pb-2 md:pb-4 border-b border-border/20 lg:border-none">
              <div className="flex flex-col gap-0.5 md:gap-1">
                <span className="text-foreground/30 text-[9px] md:text-[10px] uppercase tracking-widest font-mono">
                  Year
                </span>
        ...
        <div
          className="relative mb-12 md:mb-24 group/carousel z-0"
              ...
              allImages.map((img, idx) => (
                <motion.div
                  ...
                  animate={{
                    ...
                    rotateY:
                      idx === currentIndex ? 0 : idx < currentIndex ? 15 : -15,
                  }}
                  drag="x"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-20 mb-20 md:mb-32">
          ...
          >
            <div className="space-y-10">
              {/* Professional Compact Description */}
              <section>
                <h2 className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.3em] text-accent mb-6">
                  Executive Summary
                </h2>
              ...
              {/* Clean Professional Highlights */}
              {project.highlights && (
                <section className="pt-8 border-t border-border/10">
                  <h3 className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.3em] text-foreground/30 mb-6">
                    Key Achievements
                  </h3>
                  <div className="space-y-4">
                    {project.highlights.map(
                      (highlight: string, index: number) => (
                        ...
                        >
                          <span className="mt-2 w-1 h-1 bg-accent/40 rounded-full shrink-0" />
            ...
            <div className="bg-foreground/5 p-6 md:p-8 border border-border lg:sticky lg:top-32 space-y-8 md:space-y-12">
              <div>
                <h3 className="flex items-center gap-3 text-[11px] md:text-[12px] font-bold uppercase tracking-widest mb-4 md:mb-6 border-b border-border/40 pb-3 md:pb-4">
                  <Code size={14} className="text-accent" /> Technical
                  Infrastructure
                </h3>
                <div className="flex flex-wrap gap-1.5 md:gap-2">
                  {project.tech.map((t) => (
                    <span
                      key={t}
                      className="px-2.5 py-1 md:px-3 md:py-1.5 bg-background border border-border text-[9px] md:text-[10px] font-bold hover:border-accent transition-colors duration-300 uppercase tracking-wider"
                    >
                      {t}
                    </span>
                  ))}
        ...
        >
          ...
          <span className="relative z-10 text-foreground/30 uppercase tracking-[0.4em] text-[10px] md:text-[14px] font-bold mb-4 md:mb-8 block group-hover:text-accent transition-colors">
            Discover Next
          </span>
